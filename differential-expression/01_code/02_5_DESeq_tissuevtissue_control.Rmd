...
gene_count_matrix_clean_og <- read_csv(file.path(dat, "gene_count_matrix_clean.csv"))
...
write.table(FG_TC_treatmentinfo, file = file.path(deg, "Foot/FG_TC_treatmentinfo.csv"), row.names = FALSE)
...
write.table(FG_TC_countmatrix, file = file.path(deg, "FG_TC_countmatrix.csv"))
...
FG_TC_treatmentinfo$tissue <- factor(FG_TC_treatmentinfo$tissue, levels = c("F", "G"))
...
stopifnot(resultsNames(dds30)[2] == "tissue_G_vs_F")   # foot is the reference
...
write.table(FG_LC_treatmentinfo, file = file.path(deg, "Foot/FG_LC_treatmentinfo.csv"), row.names = FALSE)
...
write.table(FG_LC_countmatrix, file = file.path(deg, "FG_LC_countmatrix.csv"))
#doesn't save correctly, need to figure this out
...
# before "control", coefficient 2 becomes control_vs_<trt>, and every log2 fold change
# flips sign silently. The stopifnot() after DESeq() guards the coefficient name.
FG_LC_treatmentinfo$tissue <- factor(FG_LC_treatmentinfo$tissue, levels = c("F", "G"))
...
stopifnot(resultsNames(dds31)[2] == "tissue_G_vs_F")   # foot is the reference

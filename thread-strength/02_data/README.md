...
`GOOGLESHEET-*.xlsx`, `summary.xlsx` and `thread-summary-trossulus-clean.xlsx` were removed in
commit `91dea50`. The mussel-level content is replaced by `mussel-treatment-key.csv`. The
thread-level content (`pad_area`, `failure_mode`, technician `maximum_force`) lives in
`../03_analyses/thread-summary.xlsx`, with a 445-row superset recoverable from history:
